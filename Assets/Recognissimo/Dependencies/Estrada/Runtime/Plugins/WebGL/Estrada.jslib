const MicrophonePlugin = {
    ...
    Estrada_InstallCallbacks: function (permissionGrantedPtr, permissionDeniedPtr, deviceChangePtr, trackEndedPtr, newSamplesPtr) {
        state.callbacks = {
            permissionGranted: () => {{{ makeDynCall('v', 'permissionGrantedPtr') }}}(),
            permissionDenied: msg => {{{ makeDynCall('vi', 'permissionDeniedPtr') }}}(stringToPtr(msg)),
            deviceChange: () => {{{ makeDynCall('v', 'deviceChangePtr') }}}(),
            trackEnded: deviceName => {{{ makeDynCall('vi', 'trackEndedPtr') }}}(stringToPtr(deviceName)),
            newSamples: () => {{{ makeDynCall('v', 'newSamplesPtr') }}}(),
        }
    },
    ...
    Estrada_WriteSamples: function (sharedBufferBytesOffset, sharedBufferLength, from, allowOverflow) {
        ...
        const sharedBuffer = new Float32Array(HEAP8.buffer, sharedBufferBytesOffset, sharedBufferLength);
