...
{
    public class EnglishPhonemiser
    {
        ...
        public EnglishPhonemiser() => _fallbackG2P = new SimpleG2P();

        public int[] TextToPhonemeIds(CMUDict cmuDict, PiperModelConfig voiceConfig, string text)
        {
        ...
        public List<string> TextToIPASymbols(CMUDict cmuDict, PiperModelConfig voiceConfig, string text)
        {
            // --- Pre-processing Fixes ---

            // Convert newlines to a period to ensure the model pauses/ends the sentence there.
            // The spaces ensure it doesn't merge with the previous word (e.g. "End.Start").
            text = Regex.Replace(text, @"[\r\n]+", " . ");

            // Convert single or multiple dashes ("-", "--", "-------") to a comma.
            // This prevents the "ahhhhh" sound by turning the dash into a known pause token.
            text = Regex.Replace(text, @"-+", " , ");
            ...
            var phonemeIdMap = voiceConfig.GetPhonemeIdMap();

            AppendIfPresent(phonemeIdMap, "^", ipaSymbols);

            // Now that newlines are periods, this split logic will correctly separate them into sentences
        ...
        public int[] IPASymbolsToPhonemeIDs(PiperModelConfig voiceConfig, List<string> ipaSymbols)
        {
            var map = voiceConfig.GetPhonemeIdMap();
        ...
        public List<string> PhonemeIDsToIPASymbols(PiperModelConfig voiceConfig, int[] ids)
        {
            var map = voiceConfig.GetPhonemeIdMap();
